...
async function loadPage(pathname) {
    ...
    try {
        ...
        if (pathname === "/denuncia") {
            ...
            console.log("Página de denúncia carregada");
            // Seu código para a página de denúncia
            //--------codigo da pagina denuncia 
            const formDenuncia = document.getElementById("denuncia-form");

                formDenuncia.addEventListener("submit", async (e) => {

                    //e.preventDefault();

                    const dados = new FormData(formDenuncia);

                    const resposta = await fetch("/pages/denuncia.php", {
                        method: "POST",
                        body: dados
                    });
